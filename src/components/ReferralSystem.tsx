...
export const ReferralSystem = () => {
  ...
  const rewardTiers = [
    { referrals: 1, reward: "Discord Nitro Basic (1 Month)" },
    { referrals: 3, reward: "Discord Nitro Boost (1 Month)" },
    { referrals: 5, reward: "Discord Nitro Boost (3 Months)" },
    { referrals: 10, reward: "Discord Nitro Boost (6 Months)" },
    { referrals: 25, reward: "Discord Nitro Boost (1 Year)" },
  ];
        ...
        <CardContent>
          <div className="space-y-4">
            {rewardTiers.map((tier, index) => {
              ...
              const isCurrent = referralData.referrals < tier.referrals && 
                              (index === 0 || referralData.referrals >= rewardTiers[index - 1].referrals);
                ...
                >
                  <div className="flex items-center space-x-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                      isUnlocked ? 'bg-green-500' : isCurrent ? 'bg-purple-500' : 'bg-gray-600'
                    }`}>
                      {isUnlocked ? (
                        <Check className="h-4 w-4 text-white" />
                      ) : (
                        <span className="text-white text-sm font-bold">{tier.referrals}</span>
                      )}
                    </div>
                    <div>
